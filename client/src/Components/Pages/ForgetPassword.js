...
const ForgetPassword = () => {
  ...
  const handleSubmit = async () => {
    try {
      const res = await axios.post("http://localhost:5000/forget-password", data);
  ...
  const handleOtpVerification = async () => {
    try {
      const res = await axios.post("http://localhost:5000/forget-password/otp", data);
      if (res.status === 200 && res.data.success) {
        setErrorMessage(res.data.data)
        setOtpMatched(true)
        setOtp(false)
  ...
  const handleChangePassword = async () => {
    try {
      if (! await formValidator(data)) {
        const res = await axios.post("http://localhost:5000/forget-password/changePassword", data);
        if (res.status === 200 && res.data.success) {
          setErrorMessage(res.data.data)
          setOtpMatched(false)
          setOtp(false)
